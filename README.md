...
The agent calls `agent-do <tool> <command>`, gets JSON back, and reasons about it. One interface. Same pattern across all 78 tools. The agent learns the pattern once, then it works everywhere.

One interface for Claude to control the world. 78 tools. Browsers, simulators, databases, cloud platforms, design scoring, project memory — everything an AI agent needs to act, not just think.
...
This pattern holds across all 78 tools — browsers, simulators, spreadsheets, containers, Slack channels, Kubernetes clusters, MIDI devices. Same five verbs. Same JSON responses. One interface to learn, then it works everywhere.
...
```bash
agent-do browse login https://app.example.com   # Headed window opens
# Complete SSO, MFA, whatever it takes...
agent-do browse login done --save mysite         # Auth transfers to headless, saved for later
agent-do browse session load mysite              # Next time: instant authenticated access
```
...
| **Infrastructure** | docker, k8s, cloud, gcp, ci, vm, network, dns, ssh, render, vercel, supabase, cloudflare, okta | Containers, clusters, cloud, PaaS, CDN, SSO |
