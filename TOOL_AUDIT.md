...
> Full analysis of 72 tools against the gold standard pattern.
...
**Before**: 42 of 72 tools scored 0-1 on snapshot. **After P0-P3**: 27 tools now have snapshot commands. The remaining tools without snapshot are either niche hardware (midi, serial, usb) or thin wrappers where the underlying CLI already provides adequate output (dns, network).
...
When all 72 tools answer these 5 questions reliably, agent-do becomes the universal AI-to-world interface.
