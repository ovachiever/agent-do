...
> Full analysis of 76 tools against the gold standard pattern.
...
42 of 76 tools scored 0-1 on snapshot before the P0-P3 pass. 27 tools now have snapshot commands. The remaining tools without snapshot are either niche hardware (midi, serial, usb) or thin wrappers where the underlying CLI already provides adequate output (dns, network).
...
When all 76 tools answer these 5 questions reliably, agent-do becomes the universal AI-to-world interface.
