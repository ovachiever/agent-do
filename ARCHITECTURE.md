...
| `tools/agent-auth` | Python | Site-level auth orchestrator over encrypted auth bundles, browser import, and secure credentials. Profiles and encrypted session bundles live under `~/.agent-do/auth/`; provider-aware GitHub and Google adapters sit on top of the generic login path, `provider-refresh` can reuse upstream provider auth for cross-site SSO, account-chooser and consent checkpoints are persisted in session metadata, mailbox-driven email challenges can continue through `agent-email`, and validation decides whether a session is actually usable. |
