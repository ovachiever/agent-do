...
| `agent-auth` | Python | Site-level auth orchestrator over encrypted auth bundles, browser import, and secure credentials. Profiles live under `~/.agent-do/auth/`; `ensure` tries saved-session, browser-import, provider-refresh, then site-creds where configured. GitHub/Google profiles have provider-aware login handling, TOTP resolution through `agent-do creds`, cross-site SSO reuse, account-chooser/consent checkpoint handling, mailbox-driven email or SMS continuation via `agent-do email` and `agent-do sms`, explicit passkey/security-key and device-approval action-required states, and a `probe` command for classifying the live checkpoint branch plus frontmost macOS dialog state. |
