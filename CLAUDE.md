...
| `agent-coord` | Python | Project-local agent coordination mailbox. Derives stable agent identities from thread/tmux context, stores structured handoffs/questions/replies under the current repo’s local mailbox, supports inbox/ack/thread review, and adds advisory claims so multiple agents in the same project can coordinate without human copy/paste. |
