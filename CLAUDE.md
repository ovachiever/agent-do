...
| `agent-email` | Bash + Python | Email sending plus structured mailbox querying. `snapshot`, `search`, `latest`, `wait`, `get`, `export`, `code`, `link`, and `mailboxes` support mailbox-driven auth workflows with account/mailbox scoping, exact message fetch by id, explicit metadata/summary/body/source availability states, Apple Mail Envelope Index discovery, safe send/compose argument handling, and fixture-backed testing. |
