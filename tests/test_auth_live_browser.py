...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        write_executable(
            ...
            textwrap.dedent(
                ...
                if cmd == "permissions":
                    ...
                    raise SystemExit(0)
                if cmd == "frontmost":
                    print(json.dumps({"app": "Arc"}))
                    raise SystemExit(0)
                if cmd == "focus":
                    print(json.dumps({"focused": args[1] if len(args) > 1 else "Arc"}))
                    raise SystemExit(0)
                if cmd == "dialog":
        ...
        ensure_live = run(
            str(AGENT_DO),
            "+live(scope=browser,app=Arc,ttl=15m)",
            "auth",
            "ensure",
            "cloudflare",
            "--strategy",
        ...
        require(status_payload["session"]["live_browser_app"] == "Arc", f"unexpected live browser app: {status_payload}")
